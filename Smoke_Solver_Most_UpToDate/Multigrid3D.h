...
void MultiGridSolver3D<T>::m_applyOpenBoundaryCondition(Buffer3D<T> * mass, Buffer3D<T> * rhs, T h)
{
	...
	this->xk[0] = rhs;
	this->bk[0] = mass;
	float *hk = new float[this->m_max_level];
	hk[0] = (float)h;
	for(int i=1; i<this->m_max_level; i++)
	{
		this->xk[i]->setZero();
		this->bk[i]->setZero();
		this->rk[i]->setZero();
	...
	//bottom-up pass
	for(int i=0; i<this->m_max_level-1; ++i)
	{
		(*(this->rk[i])).copy(*(this->bk[i]));

		m_MonopleToMonopole(this->systemk[i+1],
							this->systemk[i],
			*(this->rk[i]),
			*(this->k[i+1]));
		hk[i+1] = 2.0*hk[i];
	}
	int compute_element = this->systemk[0].gridz * this->systemk[0].gridy;
	
	//left face
	//for (int k=0;k<systemk[0].gridz;k++)for (int j=0; j<systemk[0].gridy;j++)
	tbb::parallel_for(0,compute_element,1,[&](int thread_idx)
	{
		int j = thread_idx%(this->systemk[0].gridy);
		int k = thread_idx/this->systemk[0].gridy;
		float wx = -0.5*h;
		float wy = ((float)j + 0.5)*h;
		float wz = ((float)k + 0.5)*h;
		T potential = 0.0;
		for (int kk=0;kk<this->systemk[this->m_max_level-1].gridz; kk++)
			for (int jj=0;jj<this->systemk[this->m_max_level-1].gridy; jj++)
				for (int ii=0;ii<this->systemk[this->m_max_level-1].gridx; ii++)
				{
					potential += m_BarnesHutSummation(this->m_max_level-1,ii,jj,kk,hk[this->m_max_level-1],h,wx,wy,wz);
				}
				(*(this->xk[0]))(0,j,k) -= potential;
	});


	//right face
	compute_element = this->systemk[0].gridz * this->systemk[0].gridy;
	//for (int k=0;k<systemk[0].gridz;k++)for (int j=0; j<systemk[0].gridy;j++)
	tbb::parallel_for(0,compute_element,1,[&](int thread_idx)
	{
		int j = thread_idx%(this->systemk[0].gridy);
		int k = thread_idx/this->systemk[0].gridy;
		float wx = ((float)(this->systemk[0].gridx)+0.5)*h;
		float wy = ((float)j + 0.5)*h;
		float wz = ((float)k + 0.5)*h;
		T potential = 0.0;
		for (int kk=0;kk<this->systemk[this->m_max_level-1].gridz; kk++)
			for (int jj=0;jj<this->systemk[this->m_max_level-1].gridy; jj++)
				...
				(*(this->xk[0]))(this->systemk[0].gridx-1,j,k) -= potential;
	});


	//top face
	compute_element = this->systemk[0].gridz * this->systemk[0].gridx;
	//for (int k=0;k<systemk[0].gridz;k++)for (int i=0; i<systemk[0].gridx;i++)
	tbb::parallel_for(0,compute_element,1,[&](int thread_idx)
	{
		int i = thread_idx%this->systemk[0].gridx;
		int k = thread_idx/this->systemk[0].gridx;
		float wx = ((float)i + 0.5)*h;
		float wy = ((float)(this->systemk[0].gridy)+0.5)*h;
		float wz = ((float)k + 0.5)*h;
		T potential = 0.0;
		for (int kk=0;kk<this->systemk[this->m_max_level-1].gridz; kk++)
			for (int jj=0;jj<this->systemk[this->m_max_level-1].gridy; jj++)
				...
				(*(this->xk[0]))(i,this->systemk[0].gridy-1,k) -= potential;
	});
	//bottom face
	compute_element = this->systemk[0].gridz * this->systemk[0].gridx;
	//for (int k=0;k<systemk[0].gridz;k++)for (int i=0; i<systemk[0].gridx;i++)
	tbb::parallel_for(0,compute_element,1,[&](int thread_idx)
	{
		...
		for (int kk=0;kk<this->systemk[this->m_max_level-1].gridz; kk++)
			for (int jj=0;jj<this->systemk[this->m_max_level-1].gridy; jj++)
				...
				(*(this->xk[0]))(i,0,k) -= potential;
	});
	//front face
	compute_element = this->systemk[0].gridy * this->systemk[0].gridx;
	//for (int j=0;j<systemk[0].gridy;j++)for (int i=0; i<systemk[0].gridx;i++)
	tbb::parallel_for(0,compute_element,1,[&](int thread_idx)
	{
		int i = thread_idx%this->systemk[0].gridx;
		int j = thread_idx/this->systemk[0].gridx;
		float wx = ((float)i + 0.5)*h;
		float wy = ((float)j + 0.5)*h;
		float wz = (-0.5)*h;
		T potential = 0.0;
	...
	tbb::parallel_for(0,compute_element,1,[&](int thread_idx)
	{
		...
		float wx = ((float)i + 0.5)*h;
		float wy = ((float)j + 0.5)*h;
		float wz = ((float)(this->systemk[0].gridz) + 0.5)*h;
		T potential = 0.0;
		for (int kk=0;kk<this->systemk[this->m_max_level-1].gridz; kk++)
			for (int jj=0;jj<this->systemk[this->m_max_level-1].gridy; jj++)
				for (int ii=0;ii<this->systemk[this->m_max_level-1].gridx; ii++)
				...
				(*(this->xk[0]))(i,j,this->systemk[0].gridz-1) -= potential;
	});
	

	for(int i=0+1; i<this->m_max_level; i++)
	{
		this->xk[i]->setZero();
		this->bk[i]->setZero();
		this->rk[i]->setZero();
...
T MultiGridSolver3D<T>::m_BarnesHutSummation(int level, int i, int j, int k, T curr_h, T h0, T wx, T wy, T wz)
{
	...
	if(level==0 || test_d > 1.001*(0.5*curr_h + 0.5*h0))
	{
		...
		if(r>1e-7)
			return (*(this->bk[level]))(i,j,k)*0.07957747154/r;
		else
			return 0;
	}
