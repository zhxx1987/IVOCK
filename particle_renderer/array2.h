...
//============================================================================
// Array2 is a template class which overlays a 2D array structure on top of
// a 1D array class (by default, an STL vector, but the custom Array1 or
// WrapArray1 classes can work as well).
// The most important things to know are:
// - member variables ni and nj give the dimensions of the 2D array
//    (allowing entries (i,j) for i=0, 1, ..., ni-1 and j=0, 1, ..., nj-1)
// - entry (i,j) can be accessed (either for getting or assigning its value) with
//    the overloaded operator():  if A is an Array2<T>, then A(i,j) looks up the
//    entry in position (i,j).
// - the STL vector operations that make sense in 2D are included, generalized
//    where appropriate (e.g. resize() takes two dimensions instead of one)
// - there are some convenient typedefs such as
//    Array2<float, Array1<float> >    -> Array2f
...
struct Array2
{
   ...
   void grid_coordinates(size_type index, int& i, int& j) const
   {
      j=index/ni;
      i=index-ni*j;
   }

   size_type index(int i, int j) const
   {
      assert(i>=0 && i<ni && j>=0 && j<nj);
      return i+ni*j;
   }
