...
// For the most part std::vector operations are supported, with a few additional
// helpful member functions, though for the Wrap version note that memory is never
// allocated/deleted and constructor/destructors are never called from within the class,
// thus only shallow copies can be made, and some operations such as resize() and push_back()
// are limited.
//
// In addition, some common typedef's are defined for convenience, such as
//   Array1<float>     -> Array1f
//   WrapArray1<float> -> WrapArray1f;
// Also note: for the most part assertions are done with assert(), not exceptions.
//

//============================================================================
// Ugly template hacking to determine if a type is integral or not.
// You don't need to understand this to use the array classes.
struct Array1True {};
struct Array1False {};
